...
/**
 * Created by hyyx on 16/10/25.
 */

public class PpwWindowRelativeLayout extends RelativeLayout {

    public interface PpwWindowLayoutChangeListener {
        void onConfigurationChanged(final Configuration newConfig);
    }

    public enum PpwWindowOrientation {
    ...
    private PpwWindowLayoutChangeListener mPpwWindowLayoutChangeListener = null;

    private float mBubbleLegOffset = 0.75f;
    private PpwWindowOrientation mPpwWindowOrientation = PpwWindowOrientation.LEFT;


    public PpwWindowRelativeLayout(Context context) {
        this(context, null);
    }

    public PpwWindowRelativeLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public PpwWindowRelativeLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        init(context, attrs);
    }

    public void setPpwWindowChangeListener(PpwWindowLayoutChangeListener ppwWindowLayoutChangeListener) {
        mPpwWindowLayoutChangeListener = ppwWindowLayoutChangeListener;
    }
    ...
    protected void onConfigurationChanged(Configuration newConfig) {
        ...
        if (mPpwWindowLayoutChangeListener != null) {
            mPpwWindowLayoutChangeListener.onConfigurationChanged(newConfig);
        }
    }
    ...
    public void setBubbleParams(PpwWindowOrientation ppwWindowOrientation, final float bubbleOffset) {
        mBubbleLegOffset = bubbleOffset;
        mPpwWindowOrientation = ppwWindowOrientation;
    }
